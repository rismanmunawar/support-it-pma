    ...
    <div class="min-h-screen w-full bg-gray-100 dark:bg-gray-900 px-4 py-6">
        ...
        <form method="GET" class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div class="flex flex-wrap gap-2">
                ...
                {{ request('kategori') === null ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-200 text-gray-700 hover:bg-gray-300 dark:bg-gray-700 dark:text-white dark:hover:bg-gray-600' }}">
                    Semua
                </button>
                ...
                @php
                $isActive = request('kategori') === $kategori;
                $style = $isActive
                ? ($warnaKategori[$kategori] ?? 'bg-gray-600 hover:bg-gray-700 text-white')
                ...
                <button
                    name="kategori"
                    value="{{ $kategori }}"
                    class="px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap {{ $style }}">
                    {{ $kategori }}
                </button>
                @endforeach
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-4 hover:shadow-md transition-shadow duration-300">
                ...
                <div class="flex flex-col md:flex-row gap-4 items-start">
                    ...
                    <div class="relative w-full md:flex-[2] flex-shrink-0" x-data="{ open: false }" style="min-height:150px;">
                        <!-- <img src="{{ asset('storage/' . $item->gambar) }}" alt="Gambar" class="rounded-lg w-full h-full object-cover min-h-[150px]"> -->
