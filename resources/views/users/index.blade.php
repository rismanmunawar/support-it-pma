<x-app-layout>
    <div class="w-full h-screen px-4 py-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('users.create') }}" class="bg-pink-500 hover:bg-pink-600 text-white font-semibold px-4 py-2 rounded shadow">
                Create New User
            </a>
        </div>

        <div class="flex gap-2 items-center mb-4 relative">
            <select id="filterBy" class="w-40 border border-gray-300 rounded px-3 py-2">
            ...
            <div class="relative w-full">
                <input
                    ...
                    autocomplete="off" />
                <ul id="suggestions" class="absolute bg-white border w-full mt-1 rounded shadow z-10 hidden max-h-48 overflow-y-auto"></ul>
            </div>
        </div>
        ...
        searchInput.addEventListener('input', function() {
            ...
            fetch(`{{ route('users.search') }}?query=${encodeURIComponent(query)}&filter=${filter}`)
                ...
                .then(data => {
                    ...
                    if (data.length === 0) {
                        usersTableBody.innerHTML = '<tr><td colspan="4" class="text-center text-gray-500 px-4 py-2">Tidak ada user ditemukan</td></tr>';
                        return;
                    }
